                    ...
                    <tbody class="divide-y divide-gray-200">
                        @forelse ($teachers as $teacher)
                            <tr>
                                ...
                                <td class="px-4 py-3">{{ $teacher->classroom->name ?? '—' }}</td>
                                <td class="px-4 py-3 text-right space-x-2">
                                    {{-- <a href="{{ route('teachers.show', $teacher) }}" class="text-blue-600 hover:underline">View</a>
                                    <a href="{{ route('teachers.edit', $teacher) }}" class="text-yellow-600 hover:underline">Edit</a>
                                    <form action="{{ route('teachers.destroy', $teacher) }}" method="POST" class="inline" onsubmit="return confirm('Delete this teacher?');">
                                        @csrf
                                        @method('DELETE')
                                        <button type="submit" class="text-red-600 hover:underline">Delete</button>
                                    </form> --}}
                                    <div class="flex items-center justify-end gap-2">
                                        <a href="{{ route('teachers.show', $teachers) }}"
                                        ...
                                        <a href="{{ route('teachers.edit', $teachers) }}"
                        ...
                        @empty
                            <tr>
                                <td colspan="6" class="px-4 py-6 text-center text-gray-500">No teachers yet.</td>
                            </tr>
                        @endforelse
                    </tbody>
                </table>
